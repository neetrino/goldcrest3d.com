...
/**
 * Hip-Hop tablet tier (640px–1023px): body copy smaller than desktop overlay so it does not crowd the title.
...
export const HIPHOP_TABLET_DESCRIPTION_CLASS =
  "sm:max-w-[min(100%,calc(480px*var(--ms,1)))] sm:font-manrope sm:font-light sm:text-[calc(12px*var(--ms,1)*var(--mt,1))] sm:leading-[calc(17px*var(--ms,1)*var(--mt,1))]";
...
/** Tablet portrait overlay title (640px–1023px) — same scale as Hip-Hop / other modeling cards at `sm+`. */
...
/** Tablet portrait slot body (640px–1023px): wider than mobile overlay so tablet lines match design copy (not 155px reflow). */
...
/** Mobile (< 640px): light gap before the secure-prong line (subsection). */
