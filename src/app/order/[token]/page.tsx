...
export default async function OrderPaymentPage({ params, searchParams }: Props) {
  ...
  const paidFlag = firstQuery(sp.paid);
  const paymentFlag = firstQuery(sp.payment);
  ...
  const showPaymentSuccess = paidFlag === "1";
  const showPaymentFailed = paymentFlag === "failed";
  const showPaymentPending = paymentFlag === "pending";
  const showPaymentUnknown = paymentFlag === "unknown";
      ...
      <div className="rounded-lg border border-neutral-200 bg-white p-6 shadow-sm">
        {showPaymentSuccess && (
          <p
            ...
            role="status"
          >
            Payment received. Thank you — your balance has been updated.
          </p>
        )}
        {showPaymentFailed && (
          <p
        ...
        {showPaymentUnknown && (
          <p
            ...
            role="status"
          >
            We could not confirm the payment status. If you were charged, contact us with your order details.
          </p>
        )}
